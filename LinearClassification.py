...
net = Net(n_feature = 26, n_hidden=10, n_output = 3)

optimizer = torch.optim.SGD(net.parameters(), lr=0.0002)

file_bonus = open("Bonus2.18 final_labeled.txt", "r")
...
for t in range(epoch):
    ...
    for i in range(records.__len__()):
        ...
        tensor2 = torch.LongTensor(np.array(cube, dtype="float32"))

        b = Variable(tensor2)
        prediction = net(record).unsqueeze(0)
        loss = F.nll_loss(prediction, b)
...
torch.save(net, "net218 10000 finalclassification.pkl")

#classification 的target是第几个！！！
#太牛逼了终于知道了