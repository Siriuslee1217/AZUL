...
net = torch.load("net218 10000 finalclassification.pkl")

net.eval()
class LearningMctsPlayer(Player):
    ...
    def moveEvaluation(self, move, myPlayer, grid_reward, floorNum):
        ...
        player_state = deepcopy(myPlayer)

        if i != -1:
            j = int(myPlayer.grid_scheme[i][tgrab.tile_type])

            player_state.grid_state[i][j] = 1
            reward = grid_reward[i][j] * (tgrab.num_to_pattern_line + myPlayer.lines_number[i]) / (1 + i) + negtivePoint + self.ClassificationFromState(self.round, player_state.grid_state)/5
        else:
            reward = negtivePoint
    ...
    def PredictFromState(self, round_num, grid_state):
        result = ""
        result += str(round_num) + " "
        ...
        tensor = torch.FloatTensor(np.array(record, dtype="float32"))
        input = Variable(tensor, requires_grad=True)

        prediction = net(input).item()
        print(prediction)

        return prediction

    def ClassificationFromState(self, round_num, grid_state):
        result = ""
        result += str(round_num) + " "
        for line in grid_state:
            result += " ".join(str(i) for i in line)
        ...
        prediction = torch.argmax(net(input))
        if prediction == 0:
            return 5
        elif prediction == 1:
            return 12
